...
with DAG(
    ...
    schedule="0 0 * * 0",   # "At midnight, every Sunday"
    catchup=False,
) as dag:

    pull_data_task = PythonOperator(task_id="pull_data", python_callable=pull_data)
    train_model_task = PythonOperator(task_id="train_model", python_callable=train_model)
    push_data_task = PythonOperator(task_id="push_data", python_callable=push_data)
